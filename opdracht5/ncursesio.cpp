...
bool windowManager(void);

bool window(WINDOW *childWin);
bool closeWindow(WINDOW *childWin);
bool window_2(WINDOW *childWin);
bool closeWindow_2(WINDOW *childWin);
...
bool closeCurrentWindow(void){
	switch(activeWindow_index){
		case WINDOW_1:
			closeWindow_1(childWin_1);
		break;
	case WINDOW_2:
			closeWindow_2(childWin_2);
		break;
	}
	return false;
}
...
bool window_1(WINDOW *childWin){
	...
	wmove(childWin,5,1);
	wclrtoeol(childWin);
	sprintf(string,"WINDOW_1 ACTIVE!!");
	mvwaddstr(childWin, 3, 3, string);
	sprintf(string,"WEEK3!!");
...
bool closeWindow_1(WINDOW *childWin){
	char string[100];
	wmove(childWin,3,1);
	wclrtoeol(childWin);
	wmove(childWin,4,1);
	...
	wmove(childWin,5,1);
	wclrtoeol(childWin);
	sprintf(string,"WINDOW_2 CLOSED!!");
	mvwaddstr(childWin, 3, 3, string);
	sprintf(string,"WEEK3!!");
	mvwaddstr(childWin, 4, 3, string);
	attron(COLOR_PAIR(UNSELECTED_WIN));
...
bool window_2(WINDOW *childWin){
	...
	wmove(childWin,5,1);
	wclrtoeol(childWin);
	sprintf(string,"WINDOW_2 ACTIVE!!");
	mvwaddstr(childWin, 3, 3, string);
	sprintf(string,"WEEK3!!");
	mvwaddstr(childWin, 4, 3, string);
	attron(COLOR_PAIR(UNSELECTED_WIN));
